...
function log_workflow() {
    ...
    python -c "
import os, sys
PROJECT_PATH = os.getenv('PROJECT_PATH', '$(pwd)')
...
log_workflow_run('$step_name', '$script', '$status', '''$error_msg''')
"
}
...
function run_step() {
    ...
    if [ $exit_status -ne 0 ]; then
        ...
        log_workflow "$step_name" "$script" "error" "$error_msg"
    ...
    ## Log successful run.
    log_workflow "$step_name" "$script" "success"
    rm -f "$temp_error_file"
...
while true; do
    ...
    # run_step "8.2: Topic Map" "workflow/i3_topic_map.py" # DEPRECATED
